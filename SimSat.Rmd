...
SAT <- function (Beta=0, rangeE = 1.5, rangeR = 2.5, psill = 0.05, Nsimulation =10, Nrow = 5, Ncol = 5, deterministic = "Random", Nsample =50, SampleDesign = "Random"){
  ...
  for(i in 1:Nsimulation){
    ...
    if(SampleDesign == "Random"){
      ...
      sampleSAE = SAE[seq(1, length(SAE), 2)]
      sampleSAR = SAR[seq(1, length(SAR), 2)]
    }
...
SampleDesignvec = c("Random", "Systematic")
...
for(i in 1:nrow(params)){
  ...
  Result <-data.frame(SAT(Beta=0, rangeE = params[i, 1], rangeR = params[i, 2], psill = 0.3, Nsimulation =500, Nrow = 10, Ncol =10, deterministic = "XYgradient", Nsample = 50, SampleDesign = params[i,3])) ## new dataframe for each function call. 
