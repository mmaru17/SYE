...
SAT1 <- function (Beta=0, rangeE = 1.5, rangeR = 2.5, psill = 0.05, Nsimulation =10, Nrow = 5, Ncol = 5, deterministic = "Random", Nsample =50){
  ...
  names(output) <- c("Sampling_Random","Sampling_Systematic", "Sampling_Vertical","Sampling_TwoVertical" ,"rangeE", "rangeR")
  output
}
...
for(i in 1:nrow(params)){
  ...
  Result <-data.frame(SAT1(Beta=0, rangeE = params[i, 1], rangeR = params[i, 2], psill = 0.3, Nsimulation =500, Nrow = 10, Ncol =10, deterministic = "Random", Nsample = 100)) ## new dataframe for each function call. 
...
summary(SpatialPack::modified.ttest(SAE, SAR, xy))$p.value
