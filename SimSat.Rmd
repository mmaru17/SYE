...
SAT <- function (Beta=0, rangeE = 1.5, rangeR = 2.5, psill = 0.05, Nsimulation =10, Nrow = 5, Ncol = 5, deterministic = "Random", Nsample =50){
  ...
  Vertical_pvalues = c() # For vertical
...
rangeEvec = c(0.0000001, 3, 6) ## Variogram ranges for E
rangeRvec = c(0.0000001, 3, 6) ## Variaogram ranges for R
...
for(i in 1:nrow(params)){
  ...
  Results <- data.frame(length(which(Result$Sampling_Random <0.05))/length(Result$Sampling_Random),
                        ...
                        length(which(Result$Sampling_Stratified <0.05))/length(which(!is.na(Result$Sampling_Stratified))),
...
names(Results_total)[7] <- "Sampling_Vertical"
names(Results_total)[8] <- "Sampling_Modified_Vertical"
names(Results_total)[9] <- "Sampling_TwoVertical"
names(Results_total)[10] <- "Sampling_Modified_TwoVertical"
names(Results_total)[11] <- "rangeE"
names(Results_total)[12] <- "rangeR"
